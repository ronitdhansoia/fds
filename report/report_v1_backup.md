...
per year** — figure cited as the headline of the project dashboard.
...
2. **A corridor-level stablecoin counterfactual** that estimates the
   savings if the same flows ran over USDC/USDT rails. To our knowledge
   this is the first such per-corridor estimate published on the World
   Bank panel (§4.2, §6.3).
...
is the headline fee, but the *paid* number is the total. Ratha and the
Migration and Development Brief series [6] track aggregate
flows and corridor-level prices but do not produce a corridor-level
...
stablecoins beat traditional rails on any given corridor; Chainalysis and
similar industry reports estimate aggregate global savings in the
USD 30–50 billion per year range under optimistic assumptions. None
publish a per-corridor breakdown on the World Bank panel.
...
Figure 1 sketches the data flow. Three sources (RPW, KNOMAD BRE, World
...
renders five report PNGs at 300 DPI from the same in-memory frames. The
Next.js dashboard reads the JSON at build time — there is no runtime
backend.

![Figure 1 — System architecture: data flow from World Bank panels through the Python pipeline to the static-JSON dashboard.](figures/fig06_block_diagram.png)
...
![Figure 2 — Top 20 most expensive corridors at USD 200, 2025 Q1. Stacked TCI components: fee (amber), FX margin (dark amber), speed penalty (grey).](figures/fig01_top20_corridors.png)
...
The world map in Figure 3 shades sending countries by total annual fee
...
![Figure 3 — Annual fee burden by sending country, USD billions. Five-step amber ramp; receivers shown for context.](figures/fig02_world_map.png)
...
Fintech     & $+11.758^{*}$  & $(6.285)$ & $+1.87$  & $[-0.56,\ +24.08]$ &     182 \\
\midrule
...
The Fintech coefficient is large and positive (+11.76 pp) but only
marginally significant ($p = 0.06$); the cell contains 182 observations
and is dominated by edge-case providers because RPW classifies most
digital-first MTOs (Wise, Remitly, WorldRemit) as "Money Transfer
Operator" rather than a separate digital category. We discuss this
explicitly in §7.

![Figure 4 — Operator-class coefficients from the two-way FE regression, with 95\% CI, ranked by point estimate.](figures/fig03_operator_forest.png)
...
![Figure 5 — Stablecoin savings: corridor volume vs savings rate. Bubble size $\propto$ absolute USD savings; top-10 corridors labelled.](figures/fig04_stablecoin_scatter.png)
...
![Figure 6 — Top-10 sending countries by annual fee burden, with stablecoin counterfactual savings overlaid.](figures/fig05_diaspora_burden.png)
...
account for more than a third of the global panel-matched burden.
Italy's volume-weighted TCI (7.73%) is striking: at USD 11.69 billion of
outflow it ranks fourth by absolute burden, driven by the
disproportionate contribution of Italy → Serbia and Italy → other
Balkan / North-African corridors at headline TCI in the 12–15% range.
...
Three honest limitations bound our claims.
